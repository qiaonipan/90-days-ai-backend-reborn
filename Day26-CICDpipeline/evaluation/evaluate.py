...
try:
    ...
    print("Warning: Could not import from api.py. Make sure you're running from the project root.")
...
def evaluate_system(
    ...
    print(f"  ✓ Precision: {signal_metrics['precision']:.2%}, Recall: {signal_metrics['recall']:.2%}")
    
    # Step 2: 候选检索评估
    print("\n[2/4] Evaluating Candidate Retrieval...")
    retrieval_start = time.time()
    candidate_logs = retrieve_candidate_logs(detected_signals) if detected_signals else []
    ...
    print(f"  ✓ Root cause keyword overlap: {root_cause_metrics['keyword_overlap']:.2%}")
...
def main():
    ...
    with open(log_file, 'r', encoding='utf-8') as f:
        log_entries = [line.strip() for line in f if line.strip() and len(line.strip()) > 20]
    ...
    results = evaluate_system(
        log_entries,
        ground_truth,
        user_query="What caused the anomalies in the logs?"
    )
