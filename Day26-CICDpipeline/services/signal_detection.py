    ...
    def extract_anomaly_signals(self, log_entries: List[str]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for line in log_entries:
                ...
                if match:
                    ...
                    if ts is None:
                        ...
                        if nginx_match:
                            ...
                            try:
                                ts = pd.to_datetime(
                                    f"{date_str} {time_str}", format="%d/%b/%Y %H:%M:%S"
                                )
                            except (ValueError, TypeError):
                                ts = None
                    ...
                    if namespace is None:
                        ns_match = re.search(
                            r"\bnamespace[=:](\S+)", line, re.IGNORECASE
                        )
                        if ns_match:
                ...
                event_template = (
                    result["template_mined"] if result["template_mined"] else line
                )
            ...
            logger.info(
                f"Parsed {len(parsed_logs)} logs (HDFS: {hdfs_format_count}, K8s: {k8s_format_count})"
            )
            ...
            df["template_id"] = df["event_id"].apply(
                lambda x: f"T{x}" if x is not None else "T0"
            )
            ...
            df["is_error"] = (
                ...
                | df["message"].str.contains(
                    "ERROR|FATAL|Exception|error|fail|failed|failure",
                    case=False,
                    na=False,
                    regex=True,
                )
                ...
                | df["raw"].str.contains(
                    r"\b(?:404|500|502|503|504)\b", na=False, regex=True
                )
            )
            ...
            if len(error_rate) > 10:
                error_p95 = error_rate.rolling(
                    min(288, len(error_rate)), min_periods=1
                ).quantile(0.95)
            else:
                error_p95 = (
                    error_rate.quantile(0.95) if len(error_rate) > 0 else pd.Series([0])
                )
            ...
            top_windows = (
                ...
                if len(suspicious_windows) >= 3
                else suspicious_windows
            )

            logger.info(
                f"Found {len(suspicious_windows)} suspicious windows, returning top {len(top_windows)}"
            )
            ...
            for window_start in top_windows:
                ...
                template_counts = (
                    error_window_logs["template_id"].value_counts().head(5)
                )
                ...
                volume_spike_score = (
                    1.0 if window_start in volume_spikes[volume_spikes].index else 0.0
                )
                anomaly_score = (error_count_score * 0.8) + (volume_spike_score * 0.2)

                top_template_id = (
                    template_counts.index[0] if len(template_counts) > 0 else "UNKNOWN"
                )
                top_template_logs = error_window_logs[
                    error_window_logs["template_id"] == top_template_id
                ]
                signature = (
                    top_template_logs["template"].iloc[0]
                ...
                results.append(
                    {
                        ...
                        "count": (
                            int(template_counts.iloc[0])
                            if len(template_counts) > 0
                            else window_volume
                        ),
                        "score": round(anomaly_score, 4),
                        "templates": {
                            str(k): int(v) for k, v in template_counts.head(5).items()
                        },
                    }
                )
